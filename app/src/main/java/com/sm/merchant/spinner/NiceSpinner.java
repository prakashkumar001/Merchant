package com.sm.merchant.spinner;
...
public class NiceSpinner extends AppCompatTextView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.NiceSpinner);
		int defaultPadding = resources
				.getDimensionPixelSize(R.dimen.categorytext);

		setGravity(Gravity.CENTER_VERTICAL | Gravity.LEFT);
		setPadding(resources.getDimensionPixelSize(R.dimen.categorytext),
		...
		mHideArrow = typedArray.getBoolean(R.styleable.NiceSpinner_hideArrow,
				false);
		if (!mHideArrow) {
			Drawable basicDrawable = ContextCompat.getDrawable(context,
					R.drawable.arrow);
			int resId = typedArray.getColor(R.styleable.NiceSpinner_arrowTint,
					-1);
			...
			Drawable leftDrawable = typedArray
					.getDrawable(R.styleable.NiceSpinner_leftDrawable);

			if (leftDrawable != null)
				setCompoundDrawablesWithIntrinsicBounds(leftDrawable, null,
						mDrawable, null);
			else
				setCompoundDrawablesWithIntrinsicBounds(null, null, mDrawable,
						null);
		}
		...
		if (mAdapter != null) {
			if (position >= 0 && position <= mAdapter.getCount()) {
				...
				throw new IllegalArgumentException(
						"Position must be lower than adapter count!");
			}
		}
	}
	...
	private void animateArrow(boolean shouldRotateUp) {
		...
		ObjectAnimator animator = ObjectAnimator.ofInt(mDrawable, "level",
				start, end);
